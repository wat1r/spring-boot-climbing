...
数据库架构：
      我们需要两个表：一个用于存储关于URL映射的信息，另一个用于创建短链接的用户数据。
                                      ![image-20210326172358699](/Users/frankcooper/Library/Application Support/typora-user-images/image-20210326172358699.png)                   

      **我们应该使用什么样的数据库?**因为我们预期存储数十亿行，而且我们不需要使用对象之间的关系——像`DynamoDB`、`Cassandra`或`Riak`这样的`NoSQL`键值存储是更好的选择。`NoSQL`选择也更容易扩展。
